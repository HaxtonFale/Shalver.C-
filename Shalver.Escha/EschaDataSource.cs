...
namespace Shalver.Escha;

public class EschaDataSource : IDataSource
{
    ...
    private readonly ILogger<EschaDataSource> _logger;

    public EschaDataSource(ILogger<EschaDataSource> logger)
    {
        _logger = logger;

        _logger.LogInformation("Loading data for Atelier Escha & Logy...");
        var loader = new CsvLoader("Shalver.Escha.Data", typeof(EschaDataSource).Assembly);
        ...
        var isStack = loader.LoadCsv("ItemIsStack", true).ToDictionary(r => r[0], r => r[1] switch
        {
            "ITEM_STACK_TYPE_STACK" => true,
            "ITEM_STACK_TYPE_NONE_STACK" => false,
            _ => throw new InvalidDataException($"Unexpected value: {r[1]}")
        });
        _logger.LogDebug("Loading Item data...");
        foreach (var itemRow in loader.LoadCsv("Items", true))
        {
            var internalName = itemRow[0];
            var displayName = itemRow[1];
            ...
            var itemType = Enum.Parse<ItemType>(itemRow[5]);
            _items[internalName] = new Item(internalName, displayName, itemType, categories, isStack[internalName]);
        }
        ...
        foreach (var recipeRow in loader.LoadCsv("Ingredients", true))
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                var ingredientName = recipeRow[3 * i + 2];
                if (string.IsNullOrWhiteSpace(ingredientName)) break;
                if (recipeRow[3 * i + 1] == "TRUE")
                {
        ...
        _logger.LogInformation("Atelier Escha & Logy data successfully loaded.");
    }
    ...
    public bool HasDisassembly => true;

    public IEnumerable<Item> GetDisassemblySources(Item item) => _disassemblies
        .Values.Where(d => d.Results.Contains(item) && item.IsStackable == d.Item.IsStackable)
        .Select(d => d.Item);
